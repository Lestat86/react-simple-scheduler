...
const InputWithError = <T extends string | number,>({ value,
  ...
  const inputClasses = `
    w-full px-5 py-4 mt-1 text-scheduler-neutral-900 bg-white border rounded-lg
    transition-all duration-200 ease-in-out
    placeholder:text-scheduler-neutral-400
    ...
    min-h-[52px]
    ${hasError ? 'border-scheduler-danger-300 focus:ring-scheduler-danger-500' : 'border-scheduler-neutral-200'}
  `.trim().replace(/\s+/g, ' ')

  return (
    <div className="flex flex-col">
      <label className='font-medium text-scheduler-neutral-700 text-sm mb-1'>{label}</label>
      <input
        type='text'
      ...
      />
      {hasError && (
        <span className='text-xs text-scheduler-danger-600 mt-2 font-medium flex items-center'>
